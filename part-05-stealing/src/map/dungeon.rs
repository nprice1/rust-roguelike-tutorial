...
fn transition_to_new_map(ecs: &mut World, new_depth: i32, show_visualizer: bool) -> Vec<Map> {
    let mut builder = level_builder(new_depth, 80, 50, show_visualizer);
    builder.build_map();
    if new_depth > 1 {
    ...
    *player_position = rltk::Point::new(player_x, player_y);
...
fn transition_to_existing_map(ecs: &mut World, new_depth: i32, offset: i32) {
    ...
    for (idx, tt) in map.tiles.iter().enumerate() {
        if *tt == stair_type {
            let mut player_position = ecs.write_resource::<rltk::Point>();
            *player_position = rltk::Point::new(idx as i32 % w, idx as i32 / w);
            let mut position_components = ecs.write_storage::<Position>();
            let player_pos_comp = position_components.get_mut(*player_entity);
            if let Some(player_pos_comp) = player_pos_comp {
...
pub fn level_transition(
    ...
    if dungeon_master.get_map(new_depth).is_some() {
        ...
        std::mem::drop(dungeon_master);
        Some(transition_to_new_map(ecs, new_depth, show_visualizer))
    }
}
