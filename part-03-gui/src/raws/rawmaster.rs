...
fn parse_sounds(n: &str) -> Sounds {
    Sounds {
        file_names: n.split(';').map(|name| String::from(name)).collect(),
    }
}
...
pub fn give_all_items(ecs: &mut World, entity: Entity) {
    let raws = &super::RAWS.lock().unwrap();
    for item in raws.raws.items.iter() {
        spawn_named_item(raws, ecs, &item.name, SpawnType::Carried { by: entity });
    }
}
