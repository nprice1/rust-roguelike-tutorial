...
pub fn get_item_color(ecs: &World, item: Entity) -> rltk::RGB {
    let dm = ecs.fetch::<crate::map::MasterDungeonMap>();
    if let Some(name) = ecs.read_storage::<Name>().get(item) {
        if ecs.read_storage::<CursedItem>().get(item).is_some()
            && dm.identified_items.contains(&name.name)
        {
            return rltk::RGB::from_f32(1.0, 0.0, 0.0);
        }
    }

    if let Some(magic) = ecs.read_storage::<MagicItem>().get(item) {
        match magic.class {
            MagicItemClass::Common => return rltk::RGB::from_f32(0.5, 1.0, 0.5),
            MagicItemClass::Rare => return rltk::RGB::from_f32(0.0, 1.0, 1.0),
            MagicItemClass::Legendary => return rltk::RGB::from_f32(0.71, 0.15, 0.93),
        }
    }
    rltk::RGB::from_f32(1.0, 1.0, 1.0)
}
