...
impl Tooltip {
    ...
    fn render(&self, draw_batch: &mut rltk::DrawBatch, x: i32, y: i32) {
        let box_gray: rltk::RGB = rltk::RGB::from_hex("#999999").expect("Oops");
        let light_gray: rltk::RGB = rltk::RGB::from_hex("#DDDDDD").expect("Oops");
        let white = rltk::RGB::named(rltk::WHITE);
        let black = rltk::RGB::named(rltk::BLACK);
        draw_batch.draw_box(
            rltk::Rect::with_size(x, y, self.width() - 1, self.height() - 1),
            rltk::ColorPair::new(white, box_gray),
        );
        for (i, s) in self.lines.iter().enumerate() {
            let col = if i == 0 { white } else { light_gray };
            draw_batch.print_color(
                rltk::Point::new(x + 1, y + i as i32 + 1),
                &s,
                rltk::ColorPair::new(col, black),
            );
        }
    }
}

pub fn draw_tooltips(ecs: &World, ctx: &mut rltk::BTerm) {
    ...
    draw_batch.set(
        rltk::Point::new(arrow_x, arrow_y),
        rltk::ColorPair::new(white, box_gray),
        arrow,
    );
    ...
    draw_batch.submit(7000).expect("Failed to submit");
}
