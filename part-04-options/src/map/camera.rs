...
pub fn get_screen_bounds(ecs: &World, _ctx: &mut rltk::BTerm) -> (i32, i32, i32, i32) {
...
pub fn render_camera(ecs: &World, ctx: &mut rltk::BTerm) {
        ...
        for (x, tx) in (min_x..max_x).enumerate() {
            if tx > 0 && tx < map_width && ty > 0 && ty < map_height {
                ...
                if map.revealed_tiles[idx] {
                    ...
                    draw_batch.set(
                        rltk::Point::new(x + 1, y + 1),
                        rltk::ColorPair::new(fg, bg),
                        glyph,
                    );
                }
    ...
    for (pos, render, entity, _hidden) in data.iter() {
            ...
            for cy in 0..size.y {
                for cx in 0..size.x {
                    ...
                    if map.visible_tiles[idx] {
                        ...
                        {
                            draw_batch.set(
                                rltk::Point::new(entity_screen_x + 1, entity_screen_y + 1),
                                rltk::ColorPair::new(render.fg, render.bg),
                                render.glyph,
                            );
                        }
                    }
        ...
        if targets.get(*entity).is_some() {
            ...
            let entity_screen_y = pos.y - min_y;
            draw_batch.set(
                rltk::Point::new(entity_screen_x, entity_screen_y + 1),
                rltk::ColorPair::new(rltk::RGB::named(rltk::RED), rltk::RGB::named(rltk::YELLOW)),
                rltk::to_cp437('['),
            );
            draw_batch.set(
                rltk::Point::new(entity_screen_x + 2, entity_screen_y + 1),
                rltk::ColorPair::new(rltk::RGB::named(rltk::RED), rltk::RGB::named(rltk::YELLOW)),
                rltk::to_cp437(']'),
            );
        }
    ...
    draw_batch.submit(0).expect("Failed to submit");
}

pub fn render_debug_map(map: &Map, ctx: &mut rltk::BTerm) {
    let player_pos = rltk::Point::new(map.width / 2, map.height / 2);
    let (x_chars, y_chars) = ctx.get_char_size();
